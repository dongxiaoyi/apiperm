...
// 创建host对应的远程操作用户与密码（用户名规则：user@host），同时创建用户名与host的对应关系(待测试)
func (r *mutationResolver) CreateRemoteUserPass(ctx context.Context, input UserPassInput) (*RemoteUserPass, error) {
	...
	// 不存在则创建用户
	pass := utils.Decrypt(input.RemotePass, configs.Logger) // 密码加密
	_, err = session.Run(sql.CreateRemnoteUser(username, pass), map[string]interface{}{})
	if err != nil {
		return &RemoteUserPass{}, err
	}

	// 创建主机与远程操作用户的关系
	_, err = session.Run(sql.CreateHostRemoteUserRelation(input.Host, username), map[string]interface{}{})
	if err != nil {
		// TODO: 删除新建的远程主机用户

		return &RemoteUserPass{}, err
	}
	return &RemoteUserPass{username, "密码不可见！"}, nil
}
...
func (r *queryResolver) HostHasUser(ctx context.Context, hostName string) ([]*RemoteUserPass, error) {
	panic("not implemented")
}
